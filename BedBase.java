package com.randomrainbow.tutorialmod.blocks.bed;

import com.randomrainbow.tutorialmod.init.ModItems;
...
{
    public static final PropertyEnum<BedBase.EnumPartType> PART = PropertyEnum.<BedBase.EnumPartType>create("part", BedBase.EnumPartType.class);
    public static final PropertyBool OCCUPIED = PropertyBool.create("occupied");
    protected static final AxisAlignedBB BED_AABB = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 0.5625D, 1.0D);

    public BedBase(String name, Material material)
    {
        super(Material.CLOTH);
        this.setDefaultState(this.blockState.getBaseState().withProperty(PART, BedBase.EnumPartType.FOOT).withProperty(OCCUPIED, Boolean.valueOf(false)));
    ...
    {
        if (state.getValue(PART) == BedBase.EnumPartType.FOOT)
        {
            TileEntity tileentity = worldIn.getTileEntity(pos);

            if (tileentity instanceof TileEntityBlankBed)
            {
                EnumDyeColor enumdyecolor = ((TileEntityBlankBed)tileentity).getColor();
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        ...
        {
            if (state.getValue(PART) != BedBase.EnumPartType.HEAD)
            {
    ...
    {
        return state.getValue(PART) == BedBase.EnumPartType.FOOT ? Items.AIR : ModItems.BLANK_BED;
    }
    ...
    {
        if (player.capabilities.isCreativeMode && state.getValue(PART) == BedBase.EnumPartType.FOOT)
        {
    ...
    {
        if (state.getValue(PART) == BedBase.EnumPartType.HEAD && te instanceof TileEntityBlankBed)
        {
    ...
    public IBlockState getStateFromMeta(int meta)
    {
        ...
        return (meta & 8) > 0 ? this.getDefaultState().withProperty(PART, BedBase.EnumPartType.HEAD).withProperty(FACING, enumfacing).withProperty(OCCUPIED, Boolean.valueOf((meta & 4) > 0)) : this.getDefaultState().withProperty(PART, BedBase.EnumPartType.FOOT).withProperty(FACING, enumfacing);
    }
